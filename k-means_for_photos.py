...
lfw_people = datasets.fetch_lfw_people(min_faces_per_person=15, resize=0.4)
x = lfw_people['data']
fig = plt.figure(figsize=(8, 6))

kmeans = Kmeans()
clusters, labels = kmeans.kmeans_fit_predict(x, k=len(lfw_people['target_names']), low=0.0, high=np.max(x))
for i in range(clusters.shape[0]):
    photo = np.argmin(cdist(clusters[i].reshape(1, clusters.shape[1]), x, metric='euclidean'), axis=1)[0]
    ax = fig.add_subplot(3, 5, i + 1, xticks=[], yticks=[])
    ax.imshow(lfw_people.images[photo], cmap='bone')
    print(lfw_people.target_names[lfw_people.target[photo]])
plt.show()
